...
All server-side APIs inside [Code.js](src/Code.js) must remain **stateless** and protocol-agnostic. 
...
- Frontend files (\`src/stylesheet.html\` and \`src/javascript.html\`) **must be inlined** into \`dist/index.html\` via the compiler script [build.js](build.js).
